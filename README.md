...
- **Host 健壮性提升**：所有魔法数字提取为命名常量，便于维护与调整。
- **Server 启动容错**：为 `nanobot serve` 启动增加异常捕获，失败时自动清理状态并抛出清晰错误。
- **错误诊断优化**：统一错误消息前缀 `[dsh-tool-nanobot]`，增加上下文信息（实际值、计数等），帮助快速定位配置问题。
- 新增 `CHANGELOG.md` 文件。

### 0.3.0

- 初始发布：`nanobot_run` 工具支持 `oneshot` 与 `server` 两种模式。
- 支持 DSH Settings 持久化配置。
- 支持沙箱权限升级审批。
- 支持运行时 Skill 注册。
