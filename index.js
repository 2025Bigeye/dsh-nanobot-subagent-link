...
export function apply(ctx) {
  ...
  const TRUNCATE_MAX_LEN = 20000
  const SERVER_RETRY_COUNT = 25
  const SERVER_RETRY_DELAY_SEC = 1
  const HTTP_TIMEOUT_SEC = 110
  const JSON_DEPTH = 20
  const MODES = {
    READ_ONLY: 'read-only',
  ...
  const WIDER = {
    [MODES.READ_ONLY]: [MODES.WORKSPACE_WRITE, MODES.DANGER_FULL_ACCESS],
    [MODES.WORKSPACE_WRITE]: [MODES.DANGER_FULL_ACCESS],
  }
  ...
  function truncate(text, max = TRUNCATE_MAX_LEN) {
    const t = String(text ?? '')
  ...
  async function resolvePolicy(args, exec) {
    ...
    if (typeof args.justification !== 'string' || !args.justification.trim()) {
      throw new Error('[dsh-tool-nanobot] invalid escalation: sandbox_permissions requires a non-empty justification')
    }
    const effective = standing && standing.mode ? standing.mode : MODES.READ_ONLY
    if (!(WIDER[effective] || []).includes(requested)) {
      throw new Error('[dsh-tool-nanobot] sandbox escalation to "' + requested + '" is not strictly wider than current "' + effective + '" mode')
    }
    const approval = ctx.get('approval')
    if (approval === undefined) throw new Error('[dsh-tool-nanobot] sandbox escalation to "' + requested + '" requires approval, but no approval service is composed')
    if (!exec || exec.agent === undefined) throw new Error('[dsh-tool-nanobot] sandbox escalation to "' + requested + '" requires approval, but the call has no agent to route it through')
    ...
    if (outcome !== 'allowed-once') throw new Error('[dsh-tool-nanobot] sandbox escalation to "' + requested + '" was not approved (outcome: ' + outcome + ')')
  ...
  async function runNanobot(args, exec) {
    ...
    if (shell === undefined) throw new Error('[dsh-tool-nanobot] shell service is not available — check that the shell plugin is composed in this preset')
    const a = args && typeof args === 'object' ? args : {}
    const cfg = readConfig()
    const prompt = String(a.prompt ?? '')
    if (!prompt.trim()) throw new Error('[dsh-tool-nanobot] prompt is required')
    ...
    if (mode === 'server') {
        ...
        if (serverState.starting) {
          ...
          serverState.starting = (async () => {
            try {
              ...
              serverState.handle = null
              serverState.started = false
              throw new Error('failed to start nanobot server: ' + (err && err.message ? err.message : String(err)))
            }
          })()
      ...
      if (!isLoopback(baseUrl)) throw new Error('[dsh-tool-nanobot] serverBaseUrl must be a loopback http URL (localhost/127.0.0.1/::1); got: ' + baseUrl)
      ...
      const script = [
        ...
        '$body=$body|ConvertTo-Json -Depth ' + String(JSON_DEPTH) + ' -Compress',
        "$lastErr=''",
        'for ($i=0; $i -lt ' + String(SERVER_RETRY_COUNT) + '; $i++) {',
        '  try {',
        "    $r=Invoke-RestMethod -Uri $u -Method Post -ContentType 'application/json' -Headers $headers -Body $body -TimeoutSec " + String(HTTP_TIMEOUT_SEC) + ",
        '    break',
        '  } catch {',
        ...
        '    if ($i -eq ' + String(SERVER_RETRY_COUNT - 1) + ') { Write-Error \"nanobot server not reachable at $u after retries: $lastErr\"; exit 1 }',
        '    Start-Sleep -Seconds ' + String(SERVER_RETRY_DELAY_SEC) + ',',
        '  }',
        '}',
        "if ($r.choices -and $r.choices[0].message) { $r.choices[0].message.content } else { $r | ConvertTo-Json -Depth " + String(JSON_DEPTH) + " }",
      ].join('\n')
    ...
    if (promptCount !== 1) throw new Error('[dsh-tool-nanobot] oneshotCommand must contain exactly one {prompt} placeholder (found ' + promptCount + ')')
    const spaceIdx = template.indexOf(' ')
    const exe = spaceIdx === -1 ? template : template.slice(0, spaceIdx)
    if (safeExe(exe) === null) throw new Error('[dsh-tool-nanobot] oneshotCommand must start with a safe bare command name (no path separators, spaces, or shell metacharacters); got: ' + exe)
  ...
  const tool = defineTool({
    ...
    parameters: {
      ...
      sandbox_permissions: { type: 'string', enum: [MODES.WORKSPACE_WRITE, MODES.DANGER_FULL_ACCESS], description: 'Wider sandbox mode for the nanobot command; requires justification and user approval.' },
